...
import { sanitizeFileNameCore } from './utils/string-utils-core';
import { generateFrontmatter } from './utils/shared';
import { sendToLLM } from './utils/llm-client';
import { replacePromptVariablesInText } from './utils/prompt-utils';
...
// Background interpreter processing - uses shared DOM-free modules.
...
async function saveToObsidianFromBackground(
	...
	if (isDailyNote) {
		...
		const formattedNoteName = sanitizeFileNameCore(noteName);
...
async function processInterpreterAndSave(payload: ProcessInterpreterPayload): Promise<void> {
	...
	try {
		...
		// Generate frontmatter using shared module
		const propertyTypeMap: Record<string, string> = {};
		for (const pt of settings.propertyTypes) {
			propertyTypeMap[pt.name] = pt.type;
		}
		const frontmatter = generateFrontmatter(resolvedProperties, propertyTypeMap);
